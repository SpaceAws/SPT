...
def check_name(context, name, obj_type="MESH"):
    ...
    if obj_type=="MESH":
        ...
        prfx, base, count_sfx, side_sfx = parse_string(name)
        if prfx == "" or base == "":
            return False
...
def get_issues(context):
    ...
    for obj in context.scene.objects:
        ...
        #Location
        if obj.type in ["MESH", "ARMATURE", "CURVE"] and not obj.location == Vector((0.0,0.0,0.0)):
            validation_check[1] = 0
            loc_objects.append(obj.name)
        #Scale
        if obj.type in ["MESH", "ARMATURE", "CURVE"] and not obj.scale == Vector((1.0,1.0,1.0)):
            validation_check[2] = 0
            scale_objects.append(obj.name)
        #Rotation
        rot = (obj.rotation_euler[0], obj.rotation_euler[1], obj.rotation_euler[2])
        if obj.type in ["MESH", "ARMATURE", "CURVE"] and not rot == (0.0, 0.0, 0.0):
